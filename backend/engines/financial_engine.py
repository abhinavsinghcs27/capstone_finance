class FinancialEngine:
    ...
    def __intit__(self, profile_data: dict):
    ...
    def calculate_ratios(self) -> dict:
        """compute DTI, Saving rate, and emergency runway (in months)"""
        if self.total_income:
            dti = (self.data.get("existing_dept", 0) / self.total_income * 100)
        ...
        if self.total_expenses:
            emergency_months = (self.date.get("emergency_fund", 0) / self.total_expenses)
        else:
            emergency_months = 0
    ...
    def compute_health_score(self) -> dict:
        ...
        if ratios["emenrgency_runway_months"] < 3:
            score -= 20 
        elif ratios["emergency_runaway_monoths"] < 6:
            score -= 5 

        if ratios["saving_rate"] < 10:
            score -= 25
        elif ratios["saving_rate"] < 20:
            score -= 10
...
def analyze_portfolio(self) -> dict:
    ...
    stocks = self.data.get("stock", 0)
    mf = self.data.get("mutual_fund", 0)
    fd = self.data.get("fixed_deposit", 0)
    gold = self.data.get("gold", 0)
    others = self.data.get("other_saving", 0)
    savings = self.data.get("other_investment", 0)

    total_assets = stocks + mf + fd + gold + others + savings

    net_worth = (
        total_assets
        - self.data.get("existing_debt", 0)
    )

    if total_assets:
        equity = round(
            (stocks + mf) / total_assets * 100,
            1
        )
